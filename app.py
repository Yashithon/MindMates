...
<script>
    window.addEventListener("load", function() {
        if (window.location.href.includes("#result")) {
            const anchor = document.getElementsByName("result")[0];
            if (anchor) {
...
def predict_tag(text):
    ...
    # Check if all words are profane (must come first)
    if all(word in banned_words for word in words) and len(words) > 0:
...
with col2:
    ...
    <div class="info-card">
        ...
        <ul>
            ...
            <li>This tool is a basic educational resource created as a personal project. It is not intended for medical diagnosis or treatment. Please consult a qualified healthcare professional for medical advice.</li>
...
if predict_button and user_input.strip():
    ...
    if model is not None and vectorizer is not None:
        with st.spinner("🌿 Gently analyzing your words..."):
            ...
            if st.session_state.get("scroll") and user_input.strip():
                st.markdown("<a name='result'></a>", unsafe_allow_html=True)
                st.write("")  # needed to trigger scroll

            # Display results in two columns
            st.markdown('<div class="results-anchor" id="results"></div>', unsafe_allow_html=True)
...
<div class="footer">
    ...
    <p class="footer-disclaimer">
        This tool is a personal project made for basic educational purposes. It is not intended to provide medical advice, diagnosis, or treatment.
    </p>
</div>
""", unsafe_allow_html=True)
